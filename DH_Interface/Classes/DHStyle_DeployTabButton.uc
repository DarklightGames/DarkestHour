//==============================================================================
// Darkest Hour: Europe '44-'45
// Copyright (c) Darklight Games.  All rights reserved.
//==============================================================================

class DHStyle_DeployTabButton extends GUIStyles;

defaultproperties
{
    KeyName="DHDeployTabButton"

    Images(0)=Texture'DH_GUI_tex.button_normal'
    Images(1)=Texture'DH_GUI_tex.button_normal'
    Images(2)=Texture'DH_GUI_tex.button_normal'
    Images(3)=Texture'DH_GUI_tex.button_normal'  //pressed
    Images(4)=Texture'DH_GUI_tex.button_pressed'

    ImgColors(0)=(R=255,G=255,B=255,A=255)
    ImgColors(1)=(R=255,G=255,B=255,A=255)
    ImgColors(2)=(R=255,G=255,B=255,A=255)
    ImgColors(3)=(R=255,G=255,B=255,A=255)
    ImgColors(4)=(R=255,G=255,B=255,A=255)

    FontBKColors(0)=(R=255,G=255,B=255,A=255)
    FontBKColors(1)=(R=255,G=255,B=255,A=255)
    FontBKColors(2)=(R=255,G=255,B=255,A=255)
    FontBKColors(3)=(R=255,G=255,B=255,A=255)
    FontBKColors(4)=(R=255,G=255,B=255,A=255)

    FontColors(0)=(R=255,G=255,B=255,A=255)
    FontColors(1)=(R=255,G=255,B=255,A=255)
    FontColors(2)=(R=255,G=255,B=255,A=255)
    FontColors(3)=(R=255,G=255,B=255,A=255)
    FontColors(4)=(R=255,G=255,B=255,A=254)

    FontNames(0)="ROSmallFont"
    FontNames(1)="ROSmallFont"
    FontNames(2)="ROSmallFont"
    FontNames(3)="ROSmallFont"
    FontNames(4)="ROSmallFont"
}
