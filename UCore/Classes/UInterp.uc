...
final static function float LerpBilateral(float T, float Start, float End, float TweenStart, float TweenEnd)
{
    if (T <= Start || T >= End)
    {
        return 0.0;
    }
    else if (T <= Start + TweenStart)
    {
        return (T - Start) / TweenStart;
    }
    else if (T >= End - TweenEnd)
    {
        return (End - T) / TweenEnd;
    }
    else
    {
        return 1.0;
