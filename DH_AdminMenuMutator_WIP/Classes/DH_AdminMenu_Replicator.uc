...
var  int			MaxCharactersInMessage;
...
replication
{
	...
	reliable if (bNetInitial && Role == ROLE_Authority)
		bBypassAdminLogin, bParaDropPlayerAllowed, bShowRealismMenu, bRealismMutPresent, MaxCharactersInMessage, WarningSound;
...
function PostBeginPlay()
{
	...
	MaxCharactersInMessage	= AMMutator.MaxCharactersInMessage;
	WarningSound			= AMMutator.WarningSound;

	if (AMMutator != none && AMMutator.bDebug)
		Log("DH_AdminMenu_Replicator: bBypassAdminLogin =" @ bBypassAdminLogin @ " bParaDropPlayerAllowed =" @ bParaDropPlayerAllowed @ " bShowRealismMenu =" @ bShowRealismMenu @ 
			" bRealismMutPresent =" @ bRealismMutPresent @ " bMinesDisabled =" @ bMinesDisabled @ " MaxCharactersInMessage =" @ MaxCharactersInMessage @ " WarningSound =" @ WarningSound);
}
...
//	local  ScoreBoard  ScoreBoard; // TEMP

//	log("DH_AdminMenu_Replicator: Tick function called"); // TEMP
	...
	if (PC != none && PC.GameReplicationInfo != none)
		CreateLocalMenus();
/*
	if (PC != none) // TEST - this is saving the ref to scoreboard, spawning an alternative, fixed HUD & then setting the new HUD's ref to scoreboard
	{
		ScoreBoard = PC.myHUD.ScoreBoard;
		PC.myHUD = Spawn(class'DH_AdminMenu_HUDfix', PC);
		PC.myHUD.ScoreBoard = ScoreBoard;
	}
*/
}
...
simulated function CreateLocalMenus()
{
	...
	if (bClientDebug)
	{
		Log("DH_AdminMenu_Replicator: bParaDropPlayerAllowed =" @ bParaDropPlayerAllowed @ " bShowRealismMenu =" @ bShowRealismMenu @ 
			" bRealismMutPresent =" @ bRealismMutPresent @ " bMinesDisabled =" @ bMinesDisabled @ " MaxCharactersInMessage =" @ MaxCharactersInMessage);
...
// Replicated clientside function that passes info to client for player to receive a private message on an ROCriticalMessage background in the middle of the their screen 
// Crucial part is when the message function is called, this actor is included as the OptionalObject parameter, allowing message to access this actor's PrivateMessage variable
simulated function ClientPrivateMessage(PlayerController Receiver, PlayerReplicationInfo AdminPRI, string Message, bool bIsAdminWarning)
{
	if (Receiver != none && AdminPRI != none && Message != "")
	{
		...
		// displays a header in a message window saying "you have received a private message/warning from admin [name]" - passes the admin's PRI so we can access their name
		Receiver.ReceiveLocalizedMessage(class'DH_AdminMenu_PrivateMessageHeader', Int(bIsAdminWarning), AdminPRI);

		// displays the message in a message window below header - passes this actor as the OptionalObject so we can access our PrivateMessage string
		Receiver.ReceiveLocalizedMessage(class'DH_AdminMenu_PrivateMessage', , , , self);

		// if the message is really long this adds a 2nd message window just below the 1st, with the remainder of the message
		if (Len(PrivateMessage) > MaxCharactersInMessage)
			Receiver.ReceiveLocalizedMessage(class'DH_AdminMenu_PrivateMessageB', , , , self);

		// as a backup, display the message as white chat text at the bottom of the player's screen
		Receiver.ClientMessage(class'DH_AdminMenu_NotifyMessages'.default.AdminWarningChat @ "'" $ AdminPRI.PlayerName $ "':" @ PrivateMessage);

		// if it is an admin warning message then play a warning sound to highlight the message
		if (bIsAdminWarning)
...
simulated function PostNetReceive()
{
	...
	{
		bSavedHideCapProgress = bHideCapProgress; // save current value of bHideCapProgress so PostNetReceive can tell if it changes in future
		ClientToggleCapProgress();
	}

	if (bHidePlayerIcon != bSavedHidePlayerIcon)
	{
		bSavedHidePlayerIcon = bHidePlayerIcon;	 // save current value of bHidePlayerIcon so PostNetReceive can tell if it changes in future
		ClientTogglePlayerIcon();
	}
}
