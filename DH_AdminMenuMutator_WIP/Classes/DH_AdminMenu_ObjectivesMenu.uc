...
exec function ObjectivesMenu(string Action, optional string PlayerName)
{
	...
	if (Action != "")
	{
		if (PlayerName != "") // if we have a player name we must have got here from the actions menu for that player - so we'll return there if we press the 'previous menu' key
		{
