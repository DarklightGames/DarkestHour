...
var  DH_AdminMenu_Replicator  Replicator;           // reference to the mutator's 'helper' replicated actor, which replicates some variables used by the menus on the client
...
// Variables set in menus to tell HandleInput function how to build its command string (allows HI to be a generic function & avoids subclass redefinitions):
