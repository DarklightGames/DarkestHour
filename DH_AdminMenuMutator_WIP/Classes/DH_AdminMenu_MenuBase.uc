//=============================================================================
// DH_AdminMenu_MenuBase - by Matt UK
//=============================================================================
//
// The base for all admin menu local interactions
// These are the menus that only exist clientside, not on the server
//
//=============================================================================
...
var  DH_AdminMenu_Replicator  Replicator;			// reference to the mutator's 'helper' replicated actor, which replicates some variables that are used by the menus on the client
...
// Variables set in different menu classes to tell HandleInput function how to build its command string (allows HI to be a generic function & avoids subclass redefinitions):
var  localized string	MenuTitle;					// menu heading on the screen, localised so different language versions could be produced
...
var  bool				bUseSelectionIndexAsSuffix;	// tells HI to use the selection index number as a SelectionSuffix, i.e. add the number at the end of the command string
...
var  bool				bTreatSelectionAsOneWord;	// tells HI to replace any spaces in selected name with non-breaking spaces, which effectively makes it into 1 word but looks the same
var  bool				bKeepMenuOpen;				// if set to true in a function it prevents HI from closing the menu after the function completes, e.g. if error is encountered

// Menu labels - localised so different language versions could be produced:
...
var  config  string		AdminName, AdminPassword;	// the local player's own admin account details - if set in their config file, the menu will automatically log them in & out
...
// This event is triggered only when an interaction is created (not each time the level changes)
// But we remove menus & create new ones each round so we could use this to do setup stuff - however it is often too early (e.g. wouldn't have Replicator reference & may not have GRI)
...
// Displays the menu based on an input list - updates PosY & LineHeight for use in subclass (PosY is last drawn vertical position, allowing subclass to add to end of menu list)
...
function bool HandleInput(int keyIn, array<string> SelectionList)
{
	...
	local  string	CommandString, bDoAdminLogin; // bDoAdminLogin should be a bool but needs to be another type so can be passed as an out parameter to BuildCommandString
	...
	if (SelectionIndex < SelectionList.Length)
	{
		...
		ExecuteCommand(CommandString, bDoAdminLogin); // note that ExecuteCommand uses the coerce modifier that will force our string bDoAdminLogin to the correct bool type
...
function BuildCommandString(out string CommandString, out int SelectionIndex, out string bDoAdminLogin) // bDoAdminLogin is really a bool but they don't work with 'out' paramenters
{
	...
	// appending index number of player role or objective avoids problems where names of roles/objectives don't match between server & client, due to client running a different language
	if (bUseSelectionIndexAsSuffix)
	{
		if (SelectionIndex == 0) // special handling of index 0 to avoid problems where a failed cast of string to int (i.e. non-number) in mutator returns misleading index of 0
		{
...
simulated function ExecuteCommand(string CommandString, coerce bool bDoAdminLogin)
{
	if (bDoAdminLogin && !Replicator.bBypassAdminLogin && PC.Level.NetMode != NM_Standalone)
	{
		...
		if (bDebug)
			Log("DH_AdminMenu: auto-login for" @ AdminName $ ", execute console command =" @ CommandString);
	}
	else
	{
		ConsoleCommand(CommandString);
		...
		if (bDebug)
			Log("DH_AdminMenu: execute console command =" @ CommandString);
	}
}
...
{
//	log("AM menu int: AdminLogoutIfNecessary function called"); // TEMP
	...
	if (bMenuDidAdminLogin)
	{
		...
		if (bDebug)
			Log("DH_AdminMenu: automatic admin logout for" @ AdminName);
	}
}
...
{
	if (MessageNumber > 0)
		PC.ClientMessage(class'DH_AdminMenu_ErrorMessages'.static.AssembleMessage(MessageNumber, InsertedName));
}
...
function BuildMutateCommand(string CommandString, optional int MessageNumber)
{
	...
	if (bMenuDidAdminLogin) // if the menu automatically logged in the admin this flag will have been set to true
	{
		...
		ViewportOwner.Console.TypedStr = "Mutate logout" @ CommandString; // adds a 1st argument for the Mutate function that will act as a flag to log the admin out afterwards
	}
	else
	{
		ViewportOwner.Console.TypedStr = "Mutate nologout" @ CommandString; // adds a 1st argument for the Mutate function that will act as a flag NOT to log the admin out afterwards
	}
	...
	if (MessageNumber > 0) // display the relevant prompt/confirmation message across the admin's screen
		PC.ReceiveLocalizedMessage(class'DH_AdminMenu_AdminMessages', MessageNumber);
}
