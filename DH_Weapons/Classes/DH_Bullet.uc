...
/*        if (DH_Pawn(Other.Base).IsHumanControlled())
        {
        level.game.broadcast(instigator, "OrigLoc is "$OrigLoc);
        level.game.broadcast(instigator, "Location is "$Location);
        level.game.broadcast(instigator, "Before Vsize is "$Location - OrigLoc);
        level.game.broadcast(instigator, "After Vsize is "$VSize(Location - OrigLoc));
        level.game.broadcast(instigator, "BulletDist is "$BulletDist);
        level.game.broadcast(instigator, "DH_Pawn(Other.Base).GetTeamNum() is "$DH_Pawn(Other.Base).GetTeamNum());
        level.game.broadcast(instigator, "Instigator.GetTeamNum() is "$Instigator.GetTeamNum());
        level.game.broadcast(instigator, "WhizType is "$WhizType);
        log("OrigLoc is "$OrigLoc);
        log("Location is "$Location);
        log("Before Vsize is "$Location - OrigLoc);
        log("After Vsize is "$VSize(Location - OrigLoc));
        log("BulletDist is "$BulletDist);
        log("DH_Pawn(Other.Base).GetTeamNum() is "$DH_Pawn(Other.Base).GetTeamNum());
        log("Instigator.GetTeamNum() is "$Instigator.GetTeamNum());
        log("WhizType is "$WhizType);
        }  */
