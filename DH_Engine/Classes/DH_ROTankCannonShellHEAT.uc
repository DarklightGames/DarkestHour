...
simulated singular function HitWall(vector HitNormal, Actor Wall)
{
    ...
    Log("HEAT.HitWall: bHitWorldObject =" @ bHitWorldObject); // TEMP
...
//      if ((!Wall.bStatic && !Wall.bWorldGeometry) || RODestroyableStaticMesh(Wall) != none || Mover(Wall) != none)
    ...
    if (MaxWall > 16.0)
    {
        do
        {
            ...
            Log("HEAT.HitWall: pen do loop: Tracing using TempMaxWall =" @ TempMaxWall @ "(MaxWall =" @ MaxWall $")"); // TEMP
        ...
        Log("HEAT.HitWall: pen do loop: ended with TempMaxWall =" @ TempMaxWall @ "(MaxWall =" @ MaxWall $")"); // TEMP
    }
    else
    {
    ...
    Log("HEAT.HitWall doing pen calcs: TraceHitActor =" @ TraceHitActor); // TEMP
    if (TraceHitActor != none && SetLocation(TempHitLocation + (vect(0.5,0.0,0.0) * X)))
    {
        Log("HEAT.HitWall calling WorldPenetrationExplode: TraceHitActor =" @ TraceHitActor); // TEMP
    ...
    else Log("HEAT.HitWall NOT calling WorldPenetrationExplode: TraceHitActor =" @ TraceHitActor); // TEMP
...
{
    Log("HEAT.Explode: HitLocation =" @ HitLocation @ " HitNormal =" @ HitNormal @ " PhysicsVolume =" @ PhysicsVolume @ " bWaterVolume =" @ PhysicsVolume.bWaterVolume); // TEMP); // TEMP
    if (!bHitWorldObject)
    {
    ...
    else if (!bCollided)
    {
        ...
        BlowUp(HitLocation);
        Log("HEAT.Explode: NOT calling HandleDestruction as bHitWorldObject is true, so expecting WorldPenetrationExplode"); // TEMP
    }
    else Log("HEAT.Explode: exiting without doing anything as bCollided is true"); // TEMP
}
...
{
    Log("HEAT.WorldPenetrationExplode: HitLocation =" @ HitLocation @ " HitNormal =" @ HitNormal); // TEMP
    if (!bCollided)
    {
    ...
    else Log("HEAT.WorldPenetrationExplode: exiting without doing anything as bCollided is true"); // TEMP
}
