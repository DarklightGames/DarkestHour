...
class DH_BazookaWeapon extends DH_SemiAutoWeapon;
...
// Overridden to support cycling the bazooka aiming ranges
...
// switch the bazooka aiming ranges
...
// Switch the bazooka aiming ranges on the server
...
// Ovveriden to play the bazooka animations for different ranges
...
// Overriden so that when raised, the Bazooka has no rocket, thus requiring resupply - this prevents player from loading weapon before going into action
...
simulated function Fire(float F)
{
    ...
    {
        class'DH_BazookaWarningMsg'.Static.ClientReceive(PlayerController(Instigator.Controller),0);
        return;
    }
    else if (bUsingSights)
    {
        ...
        {
            class'DH_BazookaWarningMsg'.Static.ClientReceive(PlayerController(Instigator.Controller),1);
            return;
        }
        */
    ...
    {
        class'DH_BazookaWarningMsg'.Static.ClientReceive(PlayerController(Instigator.Controller),2);
        return;
    }
...
simulated function bool AllowReload()
{
    if (Instigator.bIsCrawling)
    {
        PlayerController(Instigator.Controller).ReceiveLocalizedMessage(class'DH_BazookaWarningMsg',4);
        return false;
    }
...
// Overwritten to play Bazooka reload message
...
/*    }
    else
    {
        if (Instigator.IsHumanControlled())
        {
            PlayerController(Instigator.Controller).ReceiveLocalizedMessage(class'DH_BazookaWarningMsg',3);
        }
    } */
}
...
/*  ROPawn(Instigator).bWeaponCanBeResupplied = true;
    DH_Pawn(Instigator).bWeaponCanBeReloaded = true;          // Maybe remove all this from GiveTo and allow BringUp() to handle it. Issue seems to affect any non-primary weapons with resupply
